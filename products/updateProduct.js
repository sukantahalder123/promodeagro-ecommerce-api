    ...
    try {
        ...
        if (productData.images && Array.isArray(productData.images)) {
            ...
            for (const image of productData.images) {
                ...
                // Validate image size using sharp
                const metadata = await sharp(imageBuffer).metadata();
                console.log(metadata.width)
                if (metadata.width < 1200) {
                    return {
                        statusCode: 400,
                        body: JSON.stringify({ message: 'Image size not valid. Minimum width should be 1200px.' }),
                    };
                }
                ...
                const s3params = {
                    Bucket: 'ecomdmsservice',
                    Key: `${productData.name}-${productData.id}-${Date.now()}`, // Unique key for each image
                    Body: imageBuffer,
            ...
            expressionAttributeValues[':images'] = imageUrls; // Store the S3 URLs in DynamoDB
            updateFbData.images = imageUrls;
        }
